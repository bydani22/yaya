...
function defaultState(){
  return {
    settings: { name: '', darkMode: matchMedia('(prefers-color-scheme: dark)').matches, notifications: true },
  ...
  if(meds.length === 0){
    ...
    meds.forEach(m=>{
      ...
      card.innerHTML = `
        ...
        <button class="check-btn ${isTaken?'done':''}" aria-label="Marcar como tomada" data-id="${m.id}">✓</button>
      `;
...
function bindForm(){
  ...
  document.getElementById('medForm').addEventListener('submit', (e)=>{
    ...
    if(editingMedId){
      ...
      if(m){ m.name=name; m.time=time; m.dose=dose; m.moment=moment; m.emoji=selectedEmoji; }
      showToast('Medicamento actualizado ✏️');
    } else {
      state.medications.push({ id: 'm_'+Date.now()+'_'+Math.random().toString(36).slice(2,7), name, time, dose, moment, emoji: selectedEmoji });
