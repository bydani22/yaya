...
function getDayRecord(key){
  ...
  if(!state.history[key].notifiedIds) state.history[key].notifiedIds = [];
  ...
  if(meds.length === 0){
    ...
    meds.forEach(m=>{
      ...
      card.innerHTML = `
        ...
        <button class="check-btn ${isTaken?'done':''}" aria-label="${m.tipo === 'inyeccion' ? (isTaken ? 'Puesta. Toca para deshacer' : 'Marcar como puesta') : (isTaken ? 'Tomada. Toca para deshacer' : 'Marcar como tomada')}" data-id="${m.id}">
          <svg viewBox="0 0 24 24" fill="none"><path d="M4 12.5L9.5 18L20 6" stroke="currentColor" stroke-width="3.4" stroke-linecap="round" stroke-linejoin="round"/></svg>
        </button>
      `;
...
/* ---------------- ALARMAS / NOTIFICACIONES ----------------
   Funcionan mientras el navegador (o la app instalada) esté
   abierto o corriendo en segundo plano. Para que suenen SIEMPRE,
   incluso con el móvil bloqueado y la app totalmente cerrada,
   hace falta la fase 2 (Capacitor + APK con alarmas del sistema).
------------------------------------------------------------- */
...
function startAlarmEngine(){
  checkAlarms();
  setInterval(checkAlarms, 20000); // cada 20 segundos
...
function checkAlarms(){
  ...
  state.medications.forEach(m=>{
    if(rec.takenIds.includes(m.id) || rec.notifiedIds.includes(m.id)) return;
    const diff = nowMinutes - toMinutes(m.time);
    // ventana de 10 minutos desde la hora exacta, para no disparar avisos viejos ni futuros
    if(diff >= 0 && diff <= 10){
      fireAlarmNotification(m);
      rec.notifiedIds.push(m.id);
      changed = true;
    }
  });
...
function fireAlarmNotification(m){
  const title = 'MediDay 💜';
  const verbo = m.tipo === 'inyeccion' ? 'pincharte' : 'tomar';
  const body = `Es hora de ${verbo}: ${m.name}${m.dose ? ' · ' + m.dose : ''}`;
  const options = {
    body, icon: 'icon.svg', badge: 'icon.svg',
    tag: 'mediday-'+m.id+'-'+todayKey(),
    vibrate: [200,100,200], requireInteraction: true
  };
  if(navigator.serviceWorker && navigator.serviceWorker.ready){
    navigator.serviceWorker.ready
      .then(reg => reg.showNotification(title, options))
      .catch(()=> safeDirectNotification(title, options));
  } else {
    safeDirectNotification(title, options);
  }
}
function safeDirectNotification(title, options){
  try{ new Notification(title, options); }catch(e){ /* silencioso */ }
}
  ...
  document.getElementById('notifToggle').addEventListener('change', (e)=>{
    ...
    if(Notification.permission === 'granted'){
      ...
      Notification.requestPermission().then(perm=>{
        if(perm === 'granted'){ checkAlarms(); showToast('Alarmas activadas 🔔'); }
        else { showToast('Sin permiso no podremos avisarte 😔'); }
      });
    } else {
